...
func (repo Repo) SetProject(ctx context.Context, p entity.Project) (uint, error) {
	...
	if p.ID > 0 {
		...
		_, err = tx.ExecContext(ctx, deleteStagesQuery, deleteStagesArgs...)
		if err != nil {
	...
	for _, stage := range p.Stages {
		...
		_, err = tx.ExecContext(ctx, insertStageQuery, insertStageArgs...)
		if err != nil {
	...
	if p.ID > 0 {
		...
		_, err = tx.ExecContext(ctx, deleteEnvVarsQuery, deleteEnvVarsArgs...)
		if err != nil {
	...
	for _, ev := range p.EnvVars {
		...
		_, err = tx.ExecContext(ctx, insertEnvVarQuery, insertEnvVarArgs...)
		if err != nil {
