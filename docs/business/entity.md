...
| Поле             | Тип       | Назначение                                                            |
| ---------------- | --------- | --------------------------------------------------------------------- |
| `ID`             | int       | Уникальный идентификатор проекта в системе                            |
...
| Поле        | Тип    | Назначение                                                             |
| ----------- | ------ | ---------------------------------------------------------------------- |
| `ProjectID` | int    | Ссылка на проект, которому принадлежит этап                            |
...
| Поле          | Тип    | Назначение                                                                          |
| ------------- | ------ | ----------------------------------------------------------------------------------- |
| `RunID`       | int    | Ссылка на запуск, которому принадлежит результат этапа                              |
...
| Поле      | Тип    | Назначение                                    |
| --------- | ------ | --------------------------------------------- |
| `RunID`   | int    | Ссылка на запуск, которому принадлежит запись |
...
| `ProjectID` | int       | Ссылка на проект, с которым связано событие                |
...
| ----------- | --------- | ------------------------------------------------------------ |
| `Type`      | EventType | Тип события (Created, Updated, Deleted, RequireAppRestart)   |
| `ProjectID` | int       | Ссылка на проект, с которым связано событие (если применимо) |
...
## Переменная окружения (EnvironmentVariable)

**Название в коде:** `EnvironmentVariable`

Переменная окружения — это именованная пара «ключ-значение», используемая для передачи конфигурации в скрипты пайплайна. Переменные бывают двух типов: глобальные (доступны всем проектам) и привязанные к проекту (доступны только конкретному проекту). Переменные могут содержать ссылки на другие переменные в формате `${VAR_NAME}`.

### Поля

| Поле                 | Тип       | Назначение                                                                 |
| -------------------- | --------- | -------------------------------------------------------------------------- |
| `ID`                 | int       | Уникальный идентификатор переменной в системе                              |
| `CreatedAt`          | time.Time | Дата и время создания переменной                                           |
| `UpdatedAt`          | time.Time | Дата и время последнего изменения значения переменной                      |
| `ProjectID`          | \*int     | Ссылка на проект (nil для глобальных переменных)                           |
| `Name`               | string    | Имя переменной окружения (например, `DATABASE_URL`, `API_KEY`)             |
| `Value`              | string    | Значение переменной окружения                                              |
| `UsesOtherVariables` | bool      | Флаг наличия ссылок на другие переменные в значении (формат `${VAR_NAME}`) |

### Типы переменных

| Тип переменной | Значение `ProjectID` | Описание                                            |
| -------------- | -------------------- | --------------------------------------------------- |
| **Глобальная** | `nil`                | Доступна всем проектам в системе                    |
| **Проекта**    | `int` (ID проекта)   | Доступна только проекту с указанным идентификатором |

### Приоритет применения переменных

При выполнении скрипта переменные применяются в следующем порядке (от низшего к высшему приоритету):

1. **Родительское окружение** (если включен флаг `WithRootEnv`) — переменные окружения пользователя/системы
2. **Статические workspace-переменные** — системные переменные (например, `WORKSPACE`)
3. **Глобальные переменные** — переменные, определённые на уровне системы
4. **Переменные проекта** — переменные, определённые для конкретного проекта (наивысший приоритет)

Переменные с более высоким приоритетом перезаписывают переменные с тем же именем из нижних уровней.
...
erDiagram
    ...
    Project ||--|{ EnvironmentVariable : "имеет переменные"
...
- Один **Проект** может иметь множество **Переменных окружения** (конфигурация)
